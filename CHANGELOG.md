...
- **Probe engine** — LLM-driven test planning and async test execution (`planner`, `runner`)
- **Test plan generation** — LLM-backed plans (Anthropic, OpenAI), local **Ollama**, **MCP tool bridge**, and rule-based fallback
- **Evaluation pipeline** — Quality reports with overall score, tool accuracy, safety, latency, and cost metrics (custom RAG-grounded scorers + LLM judge)
...
- **Docker Compose** — AgentProbe API, ChromaDB, and dashboard services (a PostgreSQL container is provisioned for planned persistence; the store is in-memory today)
